...
Look at immune function genes

```{r}
immune.spid <- (david %>% filter(p_value<0.05, category=="UP_KW_BIOLOGICAL_PROCESS") %>% #filter(stress %in% c("Cold", "Cold-OA")) %>%
             filter(grepl('DNA', process)))$genes %>% strsplit(., ",") %>% unlist() %>% as.vector() %>%
  gsub(" ", "", .)
```

Heat map of immune function related genes 
