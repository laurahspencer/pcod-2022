...
## GGSCATTER - nice formatting for paper 
...
test <- counts.annot.gadmor %>% 
  filter(spid %in%
           c("P79942", "Q32Q86", "Q63504", "B3F209", "O35710", 
             "Q5R8C5", "Q8BW74", "Q66J36", "Q92786", "O15534",
             "Q08E02", "P27925", "A4II20", "Q15466"))
...
(diffex.both.3low.6amb.filt <- subset(diffex.both.3low.6amb, rownames(diffex.both.3low.6amb) %!in% c(outs.diffex.both.3low.6amb$gene, diffex.tank.3_low, diffex.tank.6_amb)))
(diffex.both.10low.6amb.filt <- subset(diffex.both.10low.6amb, rownames(diffex.both.10low.6amb) %!in% c(outs.diffex.both.10low.6amb$gene, diffex.tank.10_low, diffex.tank.6_amb)))
...
# Simplify! Don't include the 3C vs. 10C contrast 

ggplot(n.degs %>% 
         filter(contrast %!in% c("3 vs. 10C @Amb pH", "3 vs. 10C @Low pH",
                                 "6 amb vs. 3C low", "6 amb vs. 10 low")) %>%
         mutate(contrast=fct_relevel(contrast, 
              c("Amb vs. Low pH @3C", "Amb vs. Low pH @6C", "Amb vs. Low pH @ 10C",
                "6 vs. 10C @Low pH", "6 vs. 10C @Amb pH", 
                "3 vs. 6C @Low pH","3 vs. 6C @Amb pH"))) %>%
         mutate(effect_of=case_when(
           grepl("Amb vs. Low", contrast) ~ "High pCO2",
           grepl("3 vs. 6C", contrast) ~ "Low temperature",
           grepl("6 vs. 10C", contrast) ~ "High temperature")), 
       aes(x=contrast, y=n.degs, fill=effect_of)) + 
  geom_bar(stat = "identity", color="black") +
  geom_text(hjust=-0.1, size=3.5, aes(label=paste(comma(n.degs), " (", perc, "%)", sep=""))) + 
  xlab("Contrast") + ylab("Number Differentially Expressed Genes (% of all genes)") + coord_flip() + 
  theme_minimal() + ylim(c(0,12500)) + theme(legend.position = "bottom") +
  scale_fill_manual(name=NULL, values=c("white",  "gray45", "gray80"), 
                    labels=c("Effect of High pCO2", "Effect of High Temperature", "Effect of Low Temperature")) +
...
```{r}
# Now show single stressors vs. multiple stressors 
...
ggplot(n.degs %>% 
        ...
        mutate(effect_of=fct_relevel(effect_of,
           c("High pCO2", 
             ...
             "Cold temperature + High pCO2", "Cold temperature"))), 
  ...
  theme_minimal() + ylim(c(0,12500)) + 
  theme(legend.position = "none", plot.title = element_text(size = 10)) +
  scale_fill_manual(name=NULL, values=c("gray", "firebrick4", "firebrick4", "navyblue", "navyblue")) +
  scale_alpha_manual(name=NULL, values=c(.4, .4, .7, .4, .7)) +
  guides(fill=guide_legend(nrow=3,byrow=TRUE,reverse=TRUE)) 
```
...
eulerr:::plot.euler(venn.stressors, counts = T, quantities = F, 
                      legend = T, edges=F, fontzie=8,
                      font=1, cex=1, alpha=0.6,
                      fills=list(fill=c("navyblue","firebrick4", "orange")))
```
...
# ------- "6-ambient pH vs. 3-Low pH" - effect of both cold+high pCO2 compared to optimal
print(paste("DEGs, ", names(degs[10])))
enrich.both.3low.6amb <- degs[10] %>% as.data.frame() %>% rownames_to_column("gene_gadmor") %>%
  filter(abs(select(., contains("log2FoldChange")))>0.5) %>% #filter for genes with abs(L2FC)>0.5
  left_join(gadMor.blast.GO %>% dplyr::select(gene_gadmor, gene_ontology_i_ds, spid, protein_names), by = "gene_gadmor")
enrich.both.3low.6amb %>% dplyr::select(spid) %>%  na.omit() %>% unlist() %>% as.vector() %>% write_clip(allow_non_interactive = TRUE)


# ------- "6-ambient pH vs. 3-Low pH" - effect of both warm+high pCO2 compared to optimal
print(paste("DEGs, ", names(degs[11])))
enrich.both.10low.6amb <- degs[11] %>% as.data.frame() %>% rownames_to_column("gene_gadmor") %>%
  filter(abs(select(., contains("log2FoldChange")))>0.5) %>% #filter for genes with abs(L2FC)>0.5
  left_join(gadMor.blast.GO %>% dplyr::select(gene_gadmor, gene_ontology_i_ds, spid, protein_names), by = "gene_gadmor")
...
test <- ((
  enrich.both.10low.6amb %>% filter(select(., contains("log2FoldChange"))<0.5)
  )$gene_gadmor)[1:200]
...
enriched.degs.upregulated <- bind_rows(file_list, .id = "gene_set") %>% clean_names() %>%  # Bind dataframe for each gene_set together
    #filter(category=="GOTERM_BP_DIRECT") %>%
    filter(p_value<0.05) %>%
    dplyr::rename(percent=x) %>%
    separate(term, into = c("term", "process"), sep = "~") %>%
    separate(gene_set, sep = "_", into = c("contrast", "upregulated"), remove = F) %>%
    filter(contrast %in% c("AmbvsLow.6C", "3vs6.Amb-pH","6vs10.Amb-pH", "6ambvs3low", "6ambvs10low")) %>% #keep only single and combined stressor contrasts
    mutate(contrast=factor(contrast, ordered = TRUE,
                           levels=c("AmbvsLow.6C", 
                                    "3vs6.Amb-pH","6ambvs3low", 
                                    "6vs10.Amb-pH", "6ambvs10low"))) %>% #Convert gene_set column to factor, define all gene_sets levels
  tidyr::complete(contrast, upregulated, category) %>% # add rows for gene_sets that are missing from dataframe
  mutate(upregulated=factor(upregulated, levels = c("3amb", "3low", "6amb", "6low", "10amb", "10low"), ordered = T)) %>%

  # remove upregulated treatment that doesn't exist in given contrast
  filter(!( (contrast=="AmbvsLow.6C") & (upregulated=="3amb" | upregulated=="3low" | upregulated=="10amb" | upregulated=="10low"))) %>%
  filter(!( (contrast=="3vs6.Amb") & (upregulated=="3low" | upregulated=="6low" | upregulated=="10amb" | upregulated=="10low"))) %>%
  filter(!( (contrast=="6ambvs3low") & (upregulated=="3amb" | upregulated=="6low" | upregulated=="10amb" | upregulated=="10low"))) %>%
  filter(!( (contrast=="6vs10.Amb") & (upregulated=="3amb" | upregulated=="3low" | upregulated=="6low" | upregulated=="10low"))) %>%
  filter(!( (contrast=="6ambvs10low") & (upregulated=="3amb" | upregulated=="3low" | upregulated=="6low" | upregulated=="10amb"))) %>%
...
ggplot(aes(y = process, x=contrast, col=upregulated)) + 
 ...
 scale_color_manual(name="Response\n(green=up,\nred=down)",
                    values=c(`3amb`="darkgreen", `3low`="darkgreen",
                             `6amb`="firebrick4", `6low`="darkgreen",
                             `10amb`="darkgreen", `10low`="darkgreen"),
                    labels=NULL,

             guide = guide_legend(override.aes = list(size=4))) +
  # scale_color_manual(name="Upregulated", values=c("red", "orange", "yellow", "green", "blue", "purple")) +
  scale_size("-Log10 P-value", range = c(2,8), #breaks = c(2, 5, 10),
             guide = guide_legend(override.aes = list(col="gray50"))) +
  scale_x_discrete(drop=T) + #Do drop empty factors for temperature contrasts
  theme_cleveland() + 
  theme(
        ...
        strip.text.x = element_blank()) +
  ylab("Enriched Gene Ontology (GO) Term") + xlab(NULL) +
  coord_cartesian(clip = "off", ylim = c(-3, 103)) +
  ggtitle(NULL) +
  annotate("text", x = 1, y = -1.3, label = "OA", size=2.5, col="gray15") +
  annotate("text", x = 2, y = -1.3, label = "Cold", size=2.5, col="gray15") +
  annotate("text", x = 3, y = -1.3, label = "Cold +\nOA", size=2.5, col="gray15") +
  annotate("text", x = 4, y = -1.3, label = "Warm", size=2.5, col="gray15") +
  annotate("text", x = 5, y = -1.3, label = "Warm +\nOA", size=2.5, col="gray15") +

  annotate("text", x = 3, y = 102, label = "Enriched Biological Processes", size=3, col="gray15") #+
...
```{r}
pdf(file="../results/GO-enrichment/Enriched-Uniprot-Keywords_single-vs-double-stressors_Bubble-plot.pdf", height = 8, width = 5)
...
ggplot(aes(y = process, x=contrast, col=Response)) + 
 ...
 scale_color_manual(name="Response in Stressor",
  ...
  annotate("text", x = 1, y = -1, label = "OA", size=3, col="gray15") +
  annotate("text", x = 2, y = -1, label = "Cold", size=3, col="gray15") +
  annotate("text", x = 3, y = -1, label = "Cold +\nOA", size=3, col="gray15") +
  annotate("text", x = 4, y = -1, label = "Warm", size=3, col="gray15") +
