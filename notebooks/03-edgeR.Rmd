...
filter(tab_exp_df, temperature_2_sig == "Yes") %>% 
  select(geneid) %>% distinct(geneid) %>% nrow() #1000
```
...
```
### How many genes are identified as having non-linear expression patterns according to the LRT tests? 
...
```
### How many genes are identified as having non-linear expression according to both edgeR and LRT? 527 

```{r}
edgeR_LRT_df %>% select(geneid) %>% distinct() %>% nrow()
```
...
```{r}
# Count how many gene id's exhibit pCO2 significant effect in edgeR and significant likelihood of nl effect in LRT... 527 genes
...
(edgeR_LRT_df %>%  # dataframe with genes which are deemed non-linear as per edger AND LRT
    filter(geneid %in% nl_interaction_2_sig_geneids))$geneid %>% # genes with significant interaction between temp^2 and pH as per edgeR analysis
  unique() %>% length() # count those 

# Filter down df containing sign. non-linear genes (from edger AND LRT) for only those with sign. interaction betwen temp^2 and pH
...
Exploring through gene-level plots - genes that respond non-linearly to temperature and for which pH affects that expresion:
...
Generate single plot with all genes for which there is interaction between temperature^2 and pH (those expression patterns that respond non-linearly to temperature, and that depend on pH) AND are DEGs
...
ggplot(data = nonlinear_interaction_degs_avg, 
  ...
  theme(strip.background = element_blank()) +
  labs(y = "logCPM", color = "pCO2 treatment", 
       title = "Average expression of genes with non-linear pattern across temperature\nthat is dependent on pCO2") +
  scale_color_manual(values=c("blue", "red"))
#\nConcave=158 genes; Convex=334 genes\n(edgeR, LRT & DESeq2 consensus sets)
...
There are 214 genes that respond linearly to temperature, without considering pH

```{r}
summary(DEGs_nl_temperature)
```

There are 3593 genes that have significant temperature:pH interaction
```{r}
summary(DEGs_nl_interaction)
```
```{r}
...
### How many genes are identified as having linear expression with temperature according to both edgeR and LRT? 1982 

```{r}
edgeR_LRT_linear_df %>% select(geneid) %>% distinct() %>% nrow() #quite a few! 
```

## How many genes have linear expression with temperature and significant interaction between temperature:ph?

```{r}
edgeR_LRT_linear_df %>% filter(interact_sig=="Yes") %>% select(geneid) %>% distinct() %>% nrow() #quite a few! 1,980

# How many increase with temperature?
edgeR_LRT_linear_df %>% 
  #filter(interact_sig=="Yes") %>% 
  filter(temperature_binom_linear=="Positive") %>% 
  select(geneid) %>% distinct() %>% nrow() #701

# How many decrease with temperature?
edgeR_LRT_linear_df %>% 
  #filter(interact_sig=="Yes") %>% 
  filter(temperature_binom_linear=="Negative") %>% 
  select(geneid) %>% distinct() %>% nrow() #1279
```

## How many genes have positive linear expression with temperature and significant interaction between temperature:ph and 

```{r}
edgeR_LRT_linear_df %>% filter(interact_sig=="Yes") %>% select(geneid) %>% distinct() %>% nrow() #quite a few! 
```
```{r}
# Filter down df containing sign. linear genes (from edger AND LRT) for only those with sign. interaction between temp and pH
...
ggplot(data = edgeR_LRT_linear_degs_df_avg, 
  ...
  theme(strip.background = element_blank()) +
  labs(y = "logCPM", color = "pCO2 treatment", 
       title = "Average expression of genes with linear pattern across temperature\nthat is dependent on pCO2") +
  scale_color_manual(values=c("blue", "red"))

#Negative=1,261 genes; Positive=688 genes (edgeR, LRT & DESeq2 consensus sets)
```

### Enrichment Analysis - linear and nonlinear gene sets

Here I extract 4 lists of Uniprot SPIDs for DAVID enrichment analysis. The 4 gene sets represent those that respond ... 
...
ggplot(aes(y = term_process, x=interaction(interaction, pattern), 
 ...
 facet_wrap(~category,scales="free", nrow = 2) +
  ...
  # annotate("text", x = 1, y = -1.5, label = "Decrease with\ntemperature", size=2, col="gray15") +
  # annotate("text", x = 2, y = -1.5, label = "Increase with\ntemperature", size=2, col="gray15") +
  annotate("text", x = 2.5, y = 14.5, label = "Enriched Uniprot Keywords", size=2.5, col="gray15") +
  annotate("text", x = 2.5, y = 13.75, label = "Non-linear response to temperature", size=2.5, col="gray15", fontface = 'bold') +
  annotate("text", x = 2.5, y = 13, label = "by response pattern & pH interaction effect ", size=2.5, col="gray15", fontface = 'italic')
dev.off()
```
